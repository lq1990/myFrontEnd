...
export default {
  ...
  created() {
    ...
    setInterval(() => {
      let set = this.$store.state.isSetTimer;
      ...
      setInterval(() => {
        if (this.$store.state.isOnOnce == "true") {
          // true: 开始计时， false：暂停
          this.t_begin = Date.now();
        }
      }, 200);
      if (set == "true") {
        if (this.$store.state.isOnOnce != "false") {
          this.$store.commit("updateOnOnce", "false");
        }
        let duration = Math.trunc((Date.now() - this.t_begin) / 1000);
        this.ss = duration % 60;
